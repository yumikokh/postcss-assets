...
function plainWidth(measurements) {
  return measurements.width;
}

function plainHeight(measurements) {
    ...
    .use(
      functions({
        functions: {
          ...
          pWidth: function width(path, density) {
            var normalizedPath = unquote(unescapeCss(path));
            return measure(normalizedPath, density).then(plainWidth);
          },
          pHeight: function height(path, density) {
            var normalizedPath = unquote(unescapeCss(path));
            return measure(normalizedPath, density).then(plainHeight);
          },
        },
      })
    );
